(function () {
	...
	new SingleCharInputsSet($('.page .single-char-inputs-set')[0], {
	...
	function SingleCharInputsSet(rootElement, initOptions) {
		...
		function inputOnInput(event) {
			...
			// console.log('final value inside input event:', this.getValue());
		}
		...
		function inputOnFill(event) {
			...
			if (inputIsValid) {
				...
				if (status.inputsAreForPassword) {
					input.value = '';
					inputIsValid = validateOneInput.call(this, input); // update validation again
				}
			}
